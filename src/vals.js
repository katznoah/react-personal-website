const vals = {
  "ud": [
    {
      "id": "major",
      "title": <span>Major: <strong>Computer Science</strong></span>,
      "text": <ol>Relavent Courework:
                <hr />
                <li>- Machine Organization & Assembly Language</li>
                <li>- Data Structures</li>
                <li>- Operating Systems</li>
                <li>- Database Systems</li>
                <li>- Software Engineering</li>
                <li>- Advanced Web Technologies</li>
                <li>- Web Application Security</li>
              </ol>
    },
    {
      "id": "ta",
      "title": <span>TA: <strong>Computer Science I</strong></span>,
      "text": <span>
        As a Teaching Assistant to Dr. Bart for Introduction to Computer Science I, I held weekly lab sessions for around 30 students, held office hours, and assisted in grading.
      </span>
    },
    {
      "id": "minor",
      "title": <span>Minor: <strong>Legal Studies</strong></span>,
      "text": <ol>Relavent Courework:
                <hr />
                <li>- Introduction to Law</li>
                <li>- Legal Issues of the Mass Media</li>
                <li>- Philosophy of Law</li>
                <li>- Business Law</li>
                <li>- Senior Seminal in Legal Studies</li>
              </ol>
    }
  ],
  "work": [
    {
      "id": "icims",
      "title": <span>iCIMS: <strong>Information Security Intern</strong></span>,
      "text": <span>As an Information Security Intern at iCIMS, I supported the Governance, 
        Risk, and Compliance team by owning the vendor monitoring and remediation process, developing and 
        administering security awareness training, and supporting the legal and sales departments with 
        security related issues regarding customers and vendors.</span>
    },
    {
      "id": "suth",
      "title": <span>Sutherland Global Services: <strong>Digital Consulting Intern</strong></span>,
      "text": <span>As a Digital Consulting Intern for Sutherland Global Services, I supported the 
        Digital Transformation Consulting group, reviewed and edited work proposals for clients, and 
        created and organized an online document library.</span>
    },
    {
      "id": "court",
      "title": <span>US District Court, District of NJ: <strong>Judicial Intern</strong></span>,
      "text": <span>As a Judicial Intern to The Honorable Tonianne Bongiovanni, I organized and 
        filed court documents and composed summaries of court proceedings.</span>
    }
  ]
};

export default vals;